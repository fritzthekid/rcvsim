...
%% Save a string to memory, starting at Loc
...
%% Load a string from memory, starting at Loc
...
%% Convert a string in memory to an integer
stdlib_strtol(PIDM, _Op, _Defines, _PC) ->
    ...
    Val = case string:to_integer(Str) of
        {V, []} -> V;
        _R -> throw({"rvslibs, stdlib_strtol: string not an integer", Str})
    end,
    rvscorehw:save_register(PIDM, "a0", Val),
    ok.

%% Send a message from memory to stdout
